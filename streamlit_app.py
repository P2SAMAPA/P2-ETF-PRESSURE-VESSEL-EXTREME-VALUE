...
st.set_page_config(page_title="Pressure Vessel – Extreme Value Engine", layout="wide")
...
st.markdown('<h1 style="text-align: center;">⚙️ Pressure Vessel – Extreme Value Engine</h1>', unsafe_allow_html=True)
st.markdown('<p style="text-align: center;">Weibull extreme value model with macro covariates | Burst pressure quantile</p>', unsafe_allow_html=True)

st.sidebar.markdown("## 🧮 Pressure Vessel")
...
st.sidebar.markdown(f"**Block size:** {config.BLOCK_SIZE} days")
st.sidebar.markdown(f"**Burst quantile:** {config.QUANTILE}")
...
def find_latest_json(files):
    json_files = [f for f in files if f.endswith('.json') and 'pressure_vessel_' in f]
    if not json_files:
        return None
...
def display_universe(universe_name, uni_data, window_data, window_label):
    ...
    for idx, etf in enumerate(top3):
        with cols[idx]:
            ...
            <div class="hero-card">
                <h3>{etf['ticker']}</h3>
                <p>Burst pressure: {etf['pv_score_norm']:.3f}</p>
                <p style="font-size:0.9rem;">raw quantile: {etf['raw_score']:.4f}</p>
            </div>
            """, unsafe_allow_html=True)
    with st.expander(f"Full ranking for {universe_name}"):
        df_full = pd.DataFrame(list(norm_scores.items()), columns=["Ticker", "Normalized Burst"])
        df_full["Raw Score"] = df_full["Ticker"].apply(lambda t: raw_scores[t])
        df_full = df_full.sort_values("Normalized Burst", ascending=False)
        st.dataframe(df_full, use_container_width=True)

tab1, tab2 = st.tabs(["📊 Best Window (Auto)", "🔍 Choose Window (Manual)"])

with tab1:
    st.header("⚙️ Top ETFs by Burst Pressure (Auto Best Window)")
    with st.expander("📖 Interpretation", expanded=False):
        st.markdown("""
        - **Extreme value theory** models the distribution of block maxima (e.g., monthly maximum return).
        - We fit a **Weibull distribution** with scale parameter dependent on macro variables (VIX, yields, DXY).
        - The **burst pressure** = 99.5% quantile of the Weibull conditional on current macro conditions.
        - High burst pressure → ETF can withstand extreme positive shocks → potential resilience or upside tail.
        - Low burst pressure → tail is limited, may be stable.
        """)
...
with tab2:
    ...
    st.markdown("Choose a rolling window to inspect the burst pressure scores.")
    for universe_name, uni_data in data["universes"].items():
        if not uni_data or not uni_data.get("all_windows"):
            st.warning(f"No window data for {universe_name}")
            continue
...
st.sidebar.markdown("---")
st.sidebar.caption("Pressure Vessel | Extreme value with macro covariates")
